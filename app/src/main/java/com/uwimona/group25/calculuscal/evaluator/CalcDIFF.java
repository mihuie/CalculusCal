...
public class CalcDIFF implements CalcFunctionEvaluator {
	...
	public CalcObject differentiate(CalcObject obj, CalcSymbol var) {
		...
		if (obj.getHeader().equals(CALC.TAN)) {	//	(d/dx) TAN(f(x)) = SIN(f(x)) ^ -2 * (d/dx)f(x)
			CalcFunction function = (CalcFunction)obj;
			CalcObject firstObj = function.get(0);
			return CALC.MULTIPLY.createFunction( CALC.POWER.createFunction(CALC.SIN.createFunction(firstObj),  CALC.NEG_ONE), differentiate(firstObj, var)); 
		}
